...
class UpdateProjectMembersAction(workflows.MembershipAction):
    def __init__(self, request, *args, **kwargs):
        ...
        try:
            default_role = keystone_api.get_default_role(self.request)
        ...
        if project_id:
            try:
                users_roles = keystone_api.get_project_users_roles(request,
                                                                   project_id)
            except Exception:
                exceptions.handle(request,
                                  err_msg,
...
class UpdateProjectMembers(workflows.UpdateMembersStep):
    ...
    def contribute(self, data, context):
        if data:
            try:
                roles = keystone_api.role_list(self.workflow.request)
            except Exception:
...
class UpdateProject(workflows.Workflow):
    ...
    def _get_available_roles(self, request):
        return keystone_api.role_list(request)

    def _update_project(self, request, data):
        # update project info
        try:
            project_id = data['project_id']
            return keystone_api.tenant_update(
                request,
                project_id,
                name=data['name'],
    ...
    def _remove_roles_from_user(self, request, project_id, user_id,
        ...
        for id_to_delete in current_role_ids:
            keystone_api.remove_tenant_user_role(
                request,
                project=project_id,
                user=user_id,
                role=id_to_delete)
    ...
    def _update_project_members(self, request, data, project_id):
        ...
        try:
            ...
            # can diff against it.
            users_roles = keystone_api.get_project_users_roles(
    ...
    def _get_project(self, request, data):
        try:
            project_id = data['project_id']
            return keystone_api.tenant_get(
                request,
                project_id)
        except Exception:
            exceptions.handle(request, ignore=True)
